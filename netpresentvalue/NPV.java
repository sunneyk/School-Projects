...
public class NPV {
	...
	public static double netPresentValue(double investment, int years, double payout, double r) {
		if (years == 0) {
			return -presentValue(investment, years, r);
		}
		else if (r * investment == payout){	
			return presentValue(payout, years, r) + presentValue(-investment, years-1, r);
		}
		else {
			double prev = -investment;
			for (int i = 1; i < years; i++) {
				prev += presentValue(payout, i, r);
			}
			return presentValue(payout, years, r) + prev;
		}
	}
}
